...
{  
	...
	scanf("%f", &diameter);  radius = diameter/2.0f;  
	printf("\nThe circumference is %.2f", 2.0f*Pi*radius);  
	printf("\nThe area is %.2f", Pi*radius*radius);  
	return 0;
	/*this project sole am in introduvint "const to float Pi" this ensure that across the program tye value of Pi is noy changed*/
}
